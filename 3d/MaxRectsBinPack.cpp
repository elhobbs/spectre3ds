...
namespace rbp {
		...
		int &bestShortSideFit, int &bestLongSideFit) const
	{
		...
		for (size_t i = 0; i < freeRectangles.size(); ++i)
		{
			...
			if (freeRectangles[i].width >= width && freeRectangles[i].height >= height)
			{
				...
				int shortSideFit = __min(leftoverHoriz, leftoverVert);
				int longSideFit = __max(leftoverHoriz, leftoverVert);
			...
			if (freeRectangles[i].width >= height && freeRectangles[i].height >= width)
			{
				...
				int flippedShortSideFit = __min(flippedLeftoverHoriz, flippedLeftoverVert);
				int flippedLongSideFit = __max(flippedLeftoverHoriz, flippedLeftoverVert);
	...
	int CommonIntervalLength(int i1start, int i1end, int i2start, int i2end)
	{
		if (i1end < i2start || i2end < i1start)
			return 0;
		return __min(i1end, i2end) - __max(i1start, i2start);
	}
