...
static sregion_t key_array[] = {
	...
	{ 0, 2 * 16, 1, 0, K_CAPSLOCK, key_caps },
...
void keyboard_draw_char(int x, int y, int c, u16 fg) {
	...
	u8 *fontdata = (u8 *)default_font_bin + (8 * c);
...
void keyboard_draw_region(sregion_t *region, int index, u16 c) {
	...
	{
		if ((region->key == K_CAPSLOCK && key_in_caps) || (region->key == K_SHIFT && key_in_shift)) {
			c = keyboard_bg;
		}
		else if ((region->key == K_CAPSLOCK && !key_in_caps) || (region->key == K_SHIFT && !key_in_shift)) {
			c = keyboard_fg;
		}
		ch = region->text;
		len = strlen(ch) * 8 + 4;
