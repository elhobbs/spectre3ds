...
int color_idx(oct_node root, unsigned char *pix)
{
	unsigned char i, bit;
...
oct_node nearest_color(image im, node_heap *h,int *v) {
	int i;
...
void color_quant_16(image im, int n_colors, unsigned char *palette, unsigned char *dest) {
	int i;
	unsigned char *pix = im->pix;
	node_heap heap = { 0, 0, 0 };
	...
	for (i = 1; i < heap.n; i++) {
		...
		//printf("%2d | %3llu %3llu %3llu (%d pixels)\n",
		//	i, got->r, got->g, got->b, got->count);
		palette[(i - 1) * 3 + 0] = got->r;
		palette[(i - 1) * 3 + 1] = got->g;
		palette[(i - 1) * 3 + 2] = got->b;
	}

	unsigned char *dst = dest;
	unsigned char c0, c1;
	for (i = 0, pix = im->pix; i < im->w * im->h; i += 2) {
		c0 = color_idx(root, pix);
		pix += 3;
		c1 = color_idx(root, pix);
		pix += 3;
		*dst = (c1 << 4) | c0;
		dst++;
	}
...
void color_quant_16_unpacked(image im, int n_colors, unsigned char *palette, unsigned char *dest) {
	int i;
	unsigned char *pix = im->pix;
	node_heap heap = { 0, 0, 0 };
...
void cquant(unsigned char *data, int width, int height, unsigned char *palette, unsigned char *dest) {
	image im = img_new(width, height);
	unsigned char *pix = im->pix;
	unsigned char *src = data;
	for(int i=0;i < (width*height);i++) {
		pix[0] = palette[src[0]*3+0];
		pix[1] = palette[src[0]*3+1];
		pix[2] = palette[src[0]*3+2];
...
void cquant16(unsigned char *data, int width, int height, unsigned char *palette, unsigned char *dest, unsigned char *dest_palette) {
	...
	color_quant_16(im, 15, dest_palette, dest);
...
void cquant16_unpacked(unsigned char *data,int width,int height,unsigned char *palette,unsigned char *dest,unsigned char *dest_palette) {
	...
	color_quant_16_unpacked(im, 15, dest_palette,dest);
